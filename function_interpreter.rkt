    ...
    (scheme->language
     (interpret-funcall
      ...
      (interpret-statement-list (parser file) (newenvironment) (lambda (v) v)
                               ...
                               (lambda (v env) (myerror "Uncaught exception thrown")) (lambda (env) env)) (lambda (v) v)
                               (lambda (env) (myerror "Break used outside of loop")) (lambda (env) (myerror "Continue used outside of loop"))
...
(define update-existing
  (lambda (var val environment)
    (if (exists-in-list? var (variables (car environment)))
        (begin (update-in-frame var val (topframe environment)) environment) ; added begin instead of cons here
